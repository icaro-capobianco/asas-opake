...
export const registration = ( {
    ...
    const start = ( {
        ...
        const { vk, sk } = Crypto.generateKeyPair()
...
export const authentication = ( {
    ...
    const start = ( {
        ...
        const OPRFSignature = Crypto.sign( authData.OPRFSKey, clientR )
        ...
        // Sign Response
        const signature = Crypto.sign( serverSK, response )
        return {
            response,
            signature
        }
    ...
    const finish = ( { clientSignature } : Common.Login.FinishPayload ) => {
        ...
        Crypto.validateSignature( serverR, clientVK, clientSignature )
    }
